  ...
  handler: async (ctx) => {
    ...
    completions.forEach((completion) => {
      ...
      if (completion.category) {
        byCategory[completion.category] = (byCategory[completion.category] || 0) + 1;
      }
...
export const getRecommendedChallenges = query({
  ...
  handler: async (ctx) => {
    ...
    const incompleteLabs = allLabs.filter(lab => !completedLabIds.has(lab._id));
    
    // Get categories and difficulties the user has engaged with
    const engagedCategories = new Set(completions.map(c => c.category).filter(Boolean));
    const engagedDifficulties = new Set(completions.map(c => c.difficulty).filter(Boolean));
    ...
    const scoreChallenge = (lab: any) => {
      ...
      if (lab.difficulty === "Easy" && engagedDifficulties.has("Easy")) {
        score += 3;
      } else if (lab.difficulty === "Medium" && engagedDifficulties.has("Easy")) {
        score += 4;
      } else if (lab.difficulty === "Hard" && engagedDifficulties.has("Medium")) {
        score += 4;
      }
